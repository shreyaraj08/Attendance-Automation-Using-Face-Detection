...
def find_student_name(roll_number,data):
    for student in data:
        if str(roll_number) in student:
            return student[str(roll_number)]
    return None
...
def markAttendance():
    ...
    with open(csv_file_path,mode='a',newline='') as csvfile:
        fieldnames = ['Roll No.', 'Name', 'Date','Time']
        writer = csv.DictWriter(csvfile, fieldnames=['Roll','Name','Date','Time'])
        if not file_exists:
            writer.writeheader()
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                if confidence < threshold:
                    ...
                    writer.writerow({'Roll': label, 'Name': str(name), 'Date':date, 'Time':time_now})
                    break
